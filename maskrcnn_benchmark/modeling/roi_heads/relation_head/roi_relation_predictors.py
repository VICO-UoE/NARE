...
class IMPPredictor(nn.Module):
    ...
    def forward(self, proposals, rel_pair_idxs, rel_labels, rel_binarys, roi_features, union_features, logger=None):
        ...
        obj_dists, rel_dists, mixed_rel_dists, lam, idx = self.context_layer(roi_features, proposals, union_features, rel_pair_idxs, logger)
        ...
        if self.use_bias:
            ...
            mixed_rel_dists = mixed_rel_dists + self.freq_bias.index_with_labels(pair_pred.long())
        obj_dists = obj_dists.split(num_objs, dim=0)
        rel_dists = rel_dists.split(num_rels, dim=0)
        mixed_rel_dists = mixed_rel_dists.split(num_rels, dim=0)

        # we use obj_preds instead of pred from obj_dists
        # because in decoder_rnn, preds has been through a nms stage
        add_losses = {}

        return obj_dists, rel_dists, mixed_rel_dists, lam, idx, add_losses
...
class MotifPredictor(nn.Module):
    ...
    def forward(self, proposals, rel_pair_idxs, rel_labels, rel_binarys, roi_features, union_features, logger=None):
        ...
        idx = torch.randperm(prod_rep.size(0))

        prod_rep_a, prod_rep_b = prod_rep, prod_rep[idx]

        mixed_prod_rep = l * prod_rep_a + (1 - l) * prod_rep_b        


        if self.use_vision:
            if self.union_single_not_match:
                prod_rep = prod_rep * self.up_dim(union_features)
                mixed_prod_rep = mixed_prod_rep * self.up_dim(union_features)
            else:
                prod_rep = prod_rep * union_features
                mixed_prod_rep = mixed_prod_rep * union_features 

        rel_dists = self.rel_compress(prod_rep)
        mixed_rel_dists = self.rel_compress(mixed_prod_rep)
        ...
        if self.attribute_on:
            att_dists = att_dists.split(num_objs, dim=0)
            return (obj_dists, att_dists), rel_dists,  mixed_rel_dists, l, idx,add_losses
        else:
            return obj_dists, rel_dists, mixed_rel_dists, l, idx,add_losses
...
class VCTreePredictor(nn.Module):
    ...
    def forward(self, proposals, rel_pair_idxs, rel_labels, rel_binarys, roi_features, union_features, logger=None):
        ...
        if self.union_single_not_match:
            union_features = self.up_dim(union_features)
        # union_features_a, union_features_b = union_features, union_features[idx]
        # mixed_union_features = l * union_features_a + (1 - l) * union_features_b  

        ctx_dists = self.ctx_compress(prod_rep * union_features)
        mixed_ctx_dists = self.ctx_compress(mixed_prod_rep * union_features)
        #uni_dists = self.uni_compress(self.drop(union_features))
        frq_dists = self.freq_bias.index_with_labels(pair_pred.long())

        rel_dists = ctx_dists + frq_dists
        mixed_rel_dists = mixed_ctx_dists + frq_dists
...
class CausalAnalysisPredictor(nn.Module):
    ...
    def forward(self, proposals, rel_pair_idxs, rel_labels, rel_binarys, roi_features, union_features, logger=None):
        ...
        rel_dists, mixed_rel_dists, lam, idx = self.calculate_logits(union_features, post_ctx_rep, pair_pred, use_label_dist=False)
        rel_dist_list = rel_dists.split(num_rels, dim=0)
        mixed_rel_dist_list = mixed_rel_dists.split(num_rels, dim=0)
        ...
        if self.training:
            ...
            if self.effect_type == 'TDE':   # TDE of CTX
                rel_dists = self.calculate_logits(union_features, post_ctx_rep, pair_obj_probs)[0] - self.calculate_logits(union_features, avg_ctx_rep, pair_obj_probs)[0]
            elif self.effect_type == 'NIE': # NIE of FRQ
                rel_dists = self.calculate_logits(union_features, avg_ctx_rep, pair_obj_probs)[0] - self.calculate_logits(union_features, avg_ctx_rep, avg_frq_rep)[0]
            elif self.effect_type == 'TE':  # Total Effect
                rel_dists = self.calculate_logits(union_features, post_ctx_rep, pair_obj_probs)[0] - self.calculate_logits(union_features, avg_ctx_rep, avg_frq_rep)[0]
            else:
                assert self.effect_type == 'none'
                pass
            rel_dist_list = rel_dists.split(num_rels, dim=0)

        return obj_dist_list, rel_dist_list, mixed_rel_dist_list, lam, idx,  add_losses
    ...
    def calculate_logits(self, vis_rep, ctx_rep, frq_rep, use_label_dist=True, mean_ctx=False):
        ...
        idx = torch.randperm(vis_rep.size(0))

        vis_rep_a, vis_rep_b = vis_rep, vis_rep[idx]
        ctx_rep_a, ctx_rep_b = ctx_rep, ctx_rep[idx]

        mixed_vis_rep = l * vis_rep_a + (1 - l) * vis_rep_b        
        mixed_ctx_rep = l * ctx_rep_a + (1 - l) * ctx_rep_b      

        
        mixed_vis_dists = self.vis_compress(mixed_vis_rep)
        mixed_ctx_dists = self.ctx_compress(mixed_ctx_rep)
        ...
        if self.fusion_type == 'gate':
            ...
            mixed_union_dists = mixed_vis_dists + mixed_ctx_dists + frq_dists
        else:
            print('invalid fusion type')
        
        return union_dists, mixed_union_dists, l, idx
