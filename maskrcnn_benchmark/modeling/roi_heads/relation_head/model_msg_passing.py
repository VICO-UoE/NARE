...
class IMPContext(nn.Module):
    ...
    def forward(self, x, proposals, union_features, rel_pair_idxs, logger=None):
        ...
        idx = torch.randperm(edge_factor[-1].size(0))

        edge_factor_a, edge_factor_b = edge_factor[-1], edge_factor[-1][idx]

        mixed_edge_factor = l * edge_factor_a + (1 - l) * edge_factor_b  
        mixed_rel_dists = self.rel_fc(mixed_edge_factor)

        return obj_dists, rel_dists, mixed_rel_dists, l, idx
