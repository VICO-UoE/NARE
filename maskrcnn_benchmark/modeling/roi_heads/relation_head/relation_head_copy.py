...
class ROIRelationHead(torch.nn.Module):
    ...
    def __init__(self, cfg, in_channels):
        ...
        self.two_heads = self.cfg.MODEL.ROI_RELATION_HEAD.TWO_HEADS
    ...
    def forward(self, features, proposals, targets=None, logger=None):
        ...
        if self.two_heads:
            refine_logits, relation_logits, multi_relation_logits, add_losses = self.predictor(proposals, rel_pair_idxs, rel_labels, rel_binarys, roi_features, union_features, logger)
            # for test
            # print(multi_relation_logits.shape)
            # input('enter')
            if not self.training:
                result = self.post_processor((relation_logits,refine_logits), rel_pair_idxs, proposals,multi_relation_logits=multi_relation_logits)
                return roi_features, result, {}

            loss_relation, loss_refine = self.loss_evaluator(proposals, rel_labels, relation_logits, refine_logits,multi_relation_logits=multi_relation_logits)
        else:

            refine_logits, relation_logits,mixed_relation_logits, lam, idx, add_losses = self.predictor(proposals, rel_pair_idxs, rel_labels, rel_binarys, roi_features, union_features, logger)
            ...
            loss_samples = (self.criterion_loss(relation_logits, rel_labels.long()))
            # print(loss_samples.shape)
            # print(loss_samples)
            # input('enter')
                

        #     loss_relation, loss_refine = self.loss_evaluator(proposals, rel_labels, relation_logits,mixed_relation_logits,lam, idx, refine_logits)
        ...
        return roi_features, proposals, loss_samples
